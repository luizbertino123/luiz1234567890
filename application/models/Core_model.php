...
defined('BASEPATH') OR exit('Açao não permitida');
    ...
    public function insert($tabela = NULL, $data = NULL, $get_last_id = NULL) {
        ...
        if ($tabela && $this->db->table_exists($tabela) && is_array($data)) {
            ...
            if ($this->db->affected_rows() > 0) {
                ...
                $this->db->set_flashdata('error', 'Não foi possivel salvar os dados');
            }
        } else {
    ...
    public function delete($tabela = NULL, $condicoes = NULL) {
        ...
        if ($tabela && $this->db->table_exists($tabela) && is_array($condicoes)) {
            ...
            if ($this->db->delete($tabela, $condicoes)) {
                ...
                $this->session->set_flashdata('sucesso', 'Dados excluido com sucesso!');
            } else {
                $this->db->set_flashdata('error', 'Não foi possivel excluir os dados');
            }
        } else {
            return false;
        }
