...
//         exit();
    ...
    public function core($categoria_id = NULL) {
        ...
        if (!$categoria_id) {
            ...
            $this->form_validation->set_rules('categoria_nome', 'Nome da categoria pai', 'trim|required|min_length[4]|max_length[30]|callback_valida_nome_categoria');
            ...
            if (!$categoria = $this->core_model->get_by_id('categorias', array('categoria_id' => $categoria_id))) {
                ...
                $this->form_validation->set_rules('categoria_nome', 'Nome da categoria', 'trim|required|min_length[4]|max_length[30]|callback_valida_nome_categoria');
...
//                    exit();
    ...
    public function delete($categoria_id = NULL) {
        ...
        if (!$categoria_id || !$this->core_model->get_by_id('categorias', array('categoria_id' => $categoria_id))) {
            $this->session->set_flashdata('error', 'Essa categoria filha não foi encontrada');
            redirect('restrita/categorias');
        }

        if ($this->core_model->get_by_id('categorias', array('categoria_id' => $categoria_id, 'categoria_ativa' => 1))) {
            $this->session->set_flashdata('error', 'Não é permitido excluir uma categoria filha ativa');
            redirect('restrita/categorias');
        }
